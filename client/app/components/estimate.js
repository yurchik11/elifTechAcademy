...
export default class EstimatePage extends React.Component {
    ...
    map = new google.maps.Map(document.getElementById('map'), {
     ...
     directionsDisplay = new google.maps.DirectionsRenderer({map});
		 ...
		 axios.get('/calc_time?from='+pointAtxt+'&to='+pointBtxt)
       .then(function (response) {

         console.log(response);
				 var date = new Date(response.data.data.beDelivered);
				 document.getElementById('resultQuery').value=
	...
	render() {
		...
		return (
			<div>
			 ...
			 <div id='resDivEstimate'>
			 <label for='resultQuery'>Result: </label>
			 <textarea class="form-control" rows="4" id="resultQuery"></textarea>
			 </div>
			 </form>
			</div>
		);
